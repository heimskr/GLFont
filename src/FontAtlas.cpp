  ...
  _height(0)
{
	...
	for (int i = 32; i < 128; ++i) {
		...
		// if (i == 32) {
			std::cout << i << ": _slot->bitmap.rows: " << _slot->bitmap.rows << std::endl;
			std::cout << i << ": texPos: " << texPos << std::endl;
		// }
		...
		glTexSubImage2D(
			...
			0,                  // yoffset
			1,                  // width
			_slot->bitmap.rows, // height
			GL_RED,             // format
			GL_UNSIGNED_BYTE,   // type
			_slot->bitmap.buffer             // pixels
		);
		// padding
		...
		glTexSubImage2D(GL_TEXTURE_2D, 0, texPos, 0, 1, _slot->bitmap.rows, GL_RED, GL_UNSIGNED_BYTE, _slot->bitmap.buffer); // padding
